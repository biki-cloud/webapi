...
// Selector はサーバのURLリストを受け取り、メモリなどの状態を判断し、一番負荷が少ないサーバのURLを返す。
// arg -> [http:127.0.0.1:8081,http:127.0.0.1:8082], return -> http://127.0.0.1:8082
type Selector interface {
	// Select はサーバのリストを受け取り、最適なサーバを返す。
	// 最初にサーバが生きているか確認する。
	Select(expectedAliveServers []string, confirmer serverAliveConfirmer.Confirmer, memoryGetter memoryGetter.Getter, getMemoryEndPoint, healthEndPoint string) (string, error)
}
...
// Select はサーバのリストを受け取り、最適なサーバを返す。
// 最初にサーバが生きているか確認する。
func (s *selector) Select(expectedAliveServers []string, confirmer serverAliveConfirmer.Confirmer, memoryGetter memoryGetter.Getter, getMemoryEndPoint, healthEndPoint string) (string, error) {
